...
# parameter for net
experiment_name = 'sf_ssim_aug_val_256'
# gpu_device = "cuda:0"
gpu_device = [1,2,3]
learning_rate = 1e-2
momentum = 0.9
...
image_datasets['train'] = MufocMaterialDataset(
    train_dir, transform=data_transforms, need_crop=True, need_augment=True)
image_datasets['val'] = MufocMaterialDataset(
    val_dir, transform=data_transforms, need_crop=False, need_augment=False)
dataloders = {}
...
model = UNet(n_channels=1, n_classes=1, device="cuda:1")
model.to("cuda:1")
model = nn.DataParallel(model, device_ids=gpu_device)

criterion = SSIM_SF_Loss().to("cuda:1")
# optimizer = optim.RMSprop(model.parameters(), learning_rate, momentum)
optimizer = optim.Adam(model.parameters(), learning_rate)


# optimizer = nn.DataParallel(optimizer, device_ids=gpu_device)
...
def val():
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloders['val']):
            # get the inputs
            patch1, patch2, patch_sf_y = data['last'].to("cuda:1"), data['next'].to("cuda:1"), data['sf'].to("cuda:1")
            # 先将网络中的所有梯度置0
            optimizer.zero_grad()
...
def train(epoch):
    ...
    for i, data in enumerate(dataloders['train']):
        ...
        patch1, patch2, patch_sf_y = data['last'].to(
            "cuda:1"), data['next'].to("cuda:1"), data['sf'].to("cuda:1")
